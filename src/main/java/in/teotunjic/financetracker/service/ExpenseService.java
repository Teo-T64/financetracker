...
public class ExpenseService {
    ...
    public List<ExpenseDTO> filterExpenses(LocalDate startDate, LocalDate endDate, String keyword, Sort sort){
        ProfileEntity profile = profileService.getCurrProfile();
        List<ExpenseEntity> list =  expenseRepo.findByProfile_IdAndDateBetweenAndNameContainingIgnoreCase(profile.getId(), startDate,endDate,keyword,sort);
    ...
    public List<ExpenseDTO> getExpensesForUserOnDate(Long profileId, LocalDate date){
        List<ExpenseEntity> list =  expenseRepo.findByProfile_IdAndDate(profileId,date);
        return list.stream().map(this::toExpenseDTO).toList();
    }
